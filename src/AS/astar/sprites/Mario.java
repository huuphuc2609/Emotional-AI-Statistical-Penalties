...
public class Mario extends Sprite implements Cloneable
{
    ...
    private static float marioGravity = 1;
    ...
    public void move()
    {
        ...
        if (x > world.level.xExit * world.level.getWidthCells())
        {
            win();
        }

        if (x > world.level.width * world.level.getWidthCells())
        {
            x = world.level.width * world.level.getWidthCells();
            xa = 0;
        }
