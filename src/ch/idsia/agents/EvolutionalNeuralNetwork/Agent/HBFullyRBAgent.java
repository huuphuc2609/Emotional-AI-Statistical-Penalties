 ...
 * @author Phuc
 */
public class HBFullyRBAgent extends BasicMarioAIAgent implements Agent {
    ...
    protected String name = "HBFullyRBAgent";
    ...
    public HBFullyRBAgent() {
        super("HBFullyRBAgent");
    }
    ...
    public boolean[] getAction()
    {
        ...
        double[] input = new double[nInput*nInput+8];
        int idx = 0;
        ...
        if(previousAction[0])
            input[sen.length+sen[0].length+2] = 1;
        else
            input[sen.length+sen[0].length+2] = 0;
        if(previousAction[1])
            input[sen.length+sen[0].length+3] = 1;
        else
            input[sen.length+sen[0].length+3] = 0;
        if(previousAction[2])
            input[sen.length+sen[0].length+4] = 1;
        else
            input[sen.length+sen[0].length+4] = 0;
        if(previousAction[3])
            input[sen.length+sen[0].length+5] = 1;
        else
            input[sen.length+sen[0].length+5] = 0;
        if(previousAction[4])
            input[sen.length+sen[0].length+6] = 1;
        else
            input[sen.length+sen[0].length+6] = 0;
        if(previousAction[5])
            input[sen.length+sen[0].length+7] = 1;
        else
            input[sen.length+sen[0].length+7] = 0;
        //System.out.println("");System.out.println("");
        action = nn.generateAction(input);
        
        if(sen[marioEgoRow][marioEgoRow+1] < 0)
            action[3] = true;
