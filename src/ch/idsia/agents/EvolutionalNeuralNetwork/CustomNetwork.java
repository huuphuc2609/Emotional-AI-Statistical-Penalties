 ...
 * @author Phuc
 */
public class CustomNetwork extends EvolNetwork {
          
    public int randSeed = 0;
    public boolean isFixedRandomSeed = false;
    ...
    public CustomNetwork(CustomNetwork cnw)
    {
        super(cnw);
        numSpecies = cnw.numSpecies;
        c1 = cnw.c1;
        c2 = cnw.c2;
        c3 = cnw.c3;
        listSpecies = new ArrayList<>();
        listSpecies.addAll(cnw.listSpecies);
    }
    ...
    public void mutateRemoveConnection(Genome g)
    {
        Random rand;
        if(isFixedRandomSeed)
            rand = new Random(randSeed);
            ...
            rand = new Random();
        if(!g.listConnections.isEmpty())
        {
            int idx = rand.nextInt(g.listConnections.size());
            g.listConnections.remove(idx);
        }
        if(!g.listBiasConnections.isEmpty())
        {
            int idx = rand.nextInt(g.listBiasConnections.size());
            g.listBiasConnections.remove(idx);
        }
    }
...
//                    c.w = rand.nextDouble()*2-1;
    ...
    public void mutateAllWeights(Genome g)
    {
        Random rand;
        if(isFixedRandomSeed)
            rand = new Random(randSeed);
...
//            outputList.add(newGen);
    ...
    public void mutationWithRemove(Genome g)
    {
        Random rand;
        if(isFixedRandomSeed)
            rand = new Random(randSeed);
...
//        }
        ...
        //REPRODUCTION - Mutation only, no crossover, speciations and fitness sharing
        for(int i = 0; i < listGens.size()/2; ++i)
        {
            newGeneration.add(new Genome(listGens.get(i)));
        }
        while(newGeneration.size() < getPopulation())
        {
            for(int i = 0; i < listGens.size()/2; ++i)
            {
                Genome tmp = new Genome(listGens.get(i));
                mutation(tmp);
