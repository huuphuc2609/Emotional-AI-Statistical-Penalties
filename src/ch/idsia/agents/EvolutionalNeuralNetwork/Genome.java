...
public class Genome {
    ...
    public static boolean isFixedRandomSeed = false;
    ...
    //Penalties
    public double penStand;
    public double penALLLEFT;
    public double penOnGround;
    public double penIllegalAction;
    public double penChangeAction;
    public double penCoinGained;
    public double penTotalKill;
    public double penTimeSpent;
    public double penJump;
    public double penRUNTIME;
    public double penFrontObj;
    ...
    public Genome(Genome g){
        ...
        fea = new EFeature(g.fea);
        
        penStand = g.penStand;
        penALLLEFT = g.penALLLEFT;
        penOnGround = g.penOnGround;
        penIllegalAction = g.penIllegalAction;
        penChangeAction = g.penChangeAction;
        penCoinGained = g.penCoinGained;
        penTotalKill = g.penTotalKill;
        penTimeSpent = g.penTimeSpent;
        penRUNTIME = g.penRUNTIME;
        penFrontObj = g.penFrontObj;
    }
    
    public void destruct()
    {
        listBiasConnections.clear();
        listConnections.clear();
        listHiddenNodes.clear();
        listOutputNodes.clear();
        listInputNodes.clear();
        biasInput = null;
        biasHidden = null;
        fea = null;
    }
...
//        }
    ...
    public void newBPP(double reward)
    {
        if(reward == 0)
            return;
        ...
        for(int i = 0; i < listOutputNodes.size(); ++i)
        {
            listOutputNodes.get(i).error = 1/(Math.log1p(listOutputNodes.get(i).z)*reward);
            //listOutputNodes.get(i).error = output[i] - listOutputNodes.get(i).z;
            //listOutputNodes.get(i).error *= 1/(Math.log1p((Node.derivative_sigmoid(listOutputNodes.get(i).a))*reward);
    ...
    public void newUpdateWeightBackprobagation()
    {
        //Update input layer
        for(int i = 0; i < listInputNodes.size(); ++i)
        {
